...
path = raw_input("Provide folder with benches to analyze")	# input folder

file = open("statistics_results.txt", "w") # output file

for folder in os.listdir(path):
    if("." not in folder):
        for filename in os.listdir(path+"/"+folder): # access every .txt in every folder
            if(".txt" in filename):
                ...
                max_bds = 0
                while(end <= len(array)): # sliding window with fixed size (200 elements)
